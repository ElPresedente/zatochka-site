<script setup lang="ts">
useHead({ title: 'Новый пароль — Острый край' })

const route = useRoute()
const token = computed(() => (typeof route.query.token === 'string' ? route.query.token : ''))

const password = ref('')
const passwordConfirm = ref('')
const error = ref('')
const loading = ref(false)
const done = ref(false)

async function submit() {
  error.value = ''

  if (password.value.length < 8) {
    error.value = 'Пароль должен содержать не менее 8 символов'
    return
  }
  if (password.value !== passwordConfirm.value) {
    error.value = 'Пароли не совпадают'
    return
  }

  loading.value = true
  try {
    await $fetch('/api/auth/reset-password', {
      method: 'POST',
      body: { token: token.value, password: password.value },
    })
    done.value = true
  }
  catch (e: any) {
    error.value = e?.data?.message ?? 'Не удалось сменить пароль. Запросите ссылку заново.'
  }
  finally {
    loading.value = false
  }
}
</script>

<template>
  <div class="max-w-[440px] mx-auto px-4 py-10 lg:py-16">
    <h1 class="text-2xl font-bold text-[#222] mb-6 lg:mb-8">Новый пароль</h1>

    <Transition name="fade" mode="out-in">
      <!-- Нет токена в ссылке -->
      <div v-if="!token" class="bg-white rounded-2xl shadow-sm border border-[#eee] px-5 py-6 lg:px-7 lg:py-8 text-center">
        <div class="text-4xl mb-4">⚠️</div>
        <div class="font-bold text-[#222] mb-2">Ссылка некорректна</div>
        <p class="text-sm text-[#666] mb-6">Откройте ссылку из письма восстановления пароля целиком.</p>
        <NuxtLink to="/forgot-password" class="text-brand text-sm no-underline hover:underline">Запросить новую ссылку →</NuxtLink>
      </div>

      <!-- Успех -->
      <div v-else-if="done" class="bg-white rounded-2xl shadow-sm border border-[#eee] px-5 py-6 lg:px-7 lg:py-8 text-center">
        <div class="text-4xl mb-4">✅</div>
        <div class="font-bold text-[#222] mb-2">Пароль изменён</div>
        <p class="text-sm text-[#666] mb-6">Теперь войдите с новым паролем.</p>
        <NuxtLink to="/login" class="inline-block px-7 py-3 rounded-xl bg-brand text-white font-bold text-sm hover:brightness-110 transition-all shadow-[0_3px_0_rgba(9,136,189,0.5)] no-underline">
          Войти
        </NuxtLink>
      </div>

      <!-- Форма -->
      <form v-else class="bg-white rounded-2xl shadow-sm border border-[#eee] overflow-hidden" @submit.prevent="submit">
        <div class="px-5 py-5 lg:px-7 lg:py-6 flex flex-col gap-4">
          <div>
            <label class="block text-xs font-semibold text-[#777] mb-1.5">Новый пароль <span class="text-red-400">*</span></label>
            <input
              v-model="password"
              type="password"
              autocomplete="new-password"
              class="w-full border border-[#ddd] rounded-xl px-4 py-2.5 text-sm outline-none focus:border-brand transition-colors"
            />
          </div>
          <div>
            <label class="block text-xs font-semibold text-[#777] mb-1.5">Повторите пароль <span class="text-red-400">*</span></label>
            <input
              v-model="passwordConfirm"
              type="password"
              autocomplete="new-password"
              class="w-full border border-[#ddd] rounded-xl px-4 py-2.5 text-sm outline-none focus:border-brand transition-colors"
            />
          </div>

          <Transition name="fade">
            <div v-if="error" class="text-sm text-red-500 bg-red-50 rounded-xl px-4 py-2.5">
              {{ error }}
            </div>
          </Transition>
        </div>

        <div class="px-5 py-4 lg:px-7 lg:py-5 border-t border-[#eee] flex items-center justify-between gap-3">
          <NuxtLink to="/login" class="text-sm text-[#999] no-underline hover:text-[#555] transition-colors">
            Вернуться ко входу
          </NuxtLink>
          <button
            type="submit"
            :disabled="loading || !password || !passwordConfirm"
            class="px-7 py-3 rounded-xl bg-brand text-white font-bold text-sm hover:brightness-110 transition-all shadow-[0_3px_0_rgba(9,136,189,0.5)] disabled:opacity-40 disabled:shadow-none disabled:cursor-not-allowed"
          >
            {{ loading ? 'Сохранение...' : 'Сменить пароль' }}
          </button>
        </div>
      </form>
    </Transition>
  </div>
</template>

<style scoped>
.fade-enter-active, .fade-leave-active { transition: opacity 0.25s; }
.fade-enter-from, .fade-leave-to { opacity: 0; }
</style>
